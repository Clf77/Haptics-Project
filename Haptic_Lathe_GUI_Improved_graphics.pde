// Haptic Lathe Simulator GUI - Improved Integrated Version
// Combines improved visuals with full motor control integration
// - Better visual design and layout
// - Animated spindle rotation
// - Boring bar visualization
// - Full motor control integration
// - Collision detection with haptic feedback
...
// ===== STOCK DIMENSIONS (inches -> pixels) =====
...
// Send message to Python bridge via file
...
void drawHeader() {
  ...
  float toggleW = 220;
  float toggleH = 30;
  float toggleX = W - toggleW - padding - 50;  // Space for connection indicator
...
void drawRightPanel() {
  ...
  text("Tool Load Fx: " + (toolCollision ? nf(collisionForce, 1, 1) : "0.0") + " N", innerX + 10, lineY); lineY += lineH;
...
// ===== MAIN VIEW (virtual lathe + animated stock) =====
void drawMainView() {
  float x = leftPanelW;
  float y = headerH;
  ...
  // Keep stock from overflowing the workspace
  ...
  toolCollision = (toolTipDistFromCenter < stockRadiusPx) && (toolTipXpx >= chuckX && toolTipXpx <= stockRightX);
...
// Handle motor control button clicks
...
void keyPressed() {
  ...
  if (activeField != 0) {
    ...
    // Accept only digits, decimal point
